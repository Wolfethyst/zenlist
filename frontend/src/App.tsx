    ...
    <Box minH="100vh" bgGradient="linear(135deg, #2e026d 0%, #15162c 50%, #06b6d4 100%)" color="white">
      <Container maxW="6xl" py={10}>
        <Flex justify="space-between" align="center" mb={8}>
          ...
          <HStack gap={3}>
            {user ? (
              <Button variant="outline" colorScheme="whiteAlpha" onClick={() => setUser(null)}>Sign Out</Button>
            ) : (
              <Button colorScheme="teal" variant="solid" onClick={() => setShowAuth('signup')}>Sign In / Sign Up</Button>
            )}
            <Button colorScheme="teal" variant="outline" onClick={() => navigate("/dashboard")}>Dashboard</Button>
          </HStack>
        </Flex>
        {showAuth && !user && (
          <AuthForm onAuth={u => { setUser(u); setShowAuth(false); }} />
        )}
        {!showAuth && (
          <>
